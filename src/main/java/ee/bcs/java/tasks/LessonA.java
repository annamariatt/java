...
public class LessonA {
    ...
    public static boolean e6(int x) {
        ...
        if (x % 400 == 0) {
            System.out.println("x jagub 400-ga ja on liigaasta");
            return true;
        } else if (x % 100 == 0) {
            System.out.println("x jagub 100 ja pole liigaasta");
            return false;
        } else if (x % 4 == 0) {
            System.out.println("x jagub 4 ja on liigaasta");
            return true;
        } else {
            return false;
        }
    ...
    public static boolean e9(boolean x1, boolean x2, boolean x3, boolean x4) {
        // tagasta true kui paaritu arv sisendmuutujatest on true
        int count = 0;
        if (x1) {
            count++;
        }
    ...
    public static void e15(int x[]) {
        ...
        for (int i = 1; i < x.length; i = i + 2) {
            // esimene lõik on see, mida teeme esimest korda siia loogeliste sulgudesse jõudes
            //keskmise length teemaga, ütleme, et tee nii kaua kuni i väärtus on väiksem, kui x-is olevate elementide arv, juhul kui teine tingimus on tõene
            //kolmas on see, mida tehakse iga kord, kui oleme üks kord need asjad ära teineud, ehk tsükli lõpus
            x[1] = x[i - 1];
        }
        x[1] = x[0]; //ülemine kood teeb rida rea haaval sama, mis siin, aga tsüklina
        x[3] = x[2];
        x[5] = x[4];
        x[7] = x[6];
    }
